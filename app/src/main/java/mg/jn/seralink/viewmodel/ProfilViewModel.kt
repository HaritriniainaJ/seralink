...
class ProfilViewModel(private val dataStore: TokenDataStore) : ViewModel() {
    ...
    private val _reviewAverage = MutableStateFlow(0.0)
    val reviewAverage: StateFlow<Double> = _reviewAverage
    ...
    fun loadUserReviews(userId: Int) {
        viewModelScope.launch {
            try {
                val token = dataStore.getToken() ?: return@launch
                val response = RetrofitClient.api.getUserReviews("Bearer $token", userId)
                if (response.isSuccessful) {
                    val body = response.body()!!
                    _userReviews.value = body.reviews
                    _reviewAverage.value = body.average
                    _reviewCount.value = body.count
                }
            } catch (e: Exception) {
                // silencieux
            }
